...
interface ScanResult {
  discoveredUrls: { url: string; index: number }[];
  vulnerabilities: { url: string; vulnerabilities: string[] }[];
  message?: string;
  error?: string;
}
...
const scanResultsCache: Record<string, ScanResult> = {};
...
const getSeededRandomItems = (array: string[], min: number, max: number, randomFn: () => number): string[] => {
...
const getSeededRandomNumber = (min: number, max: number, randomFn: () => number): number => {
  return Math.floor(randomFn() * (max - min + 1)) + min;
};

// Function to simulate scanning progress
const simulateScanningProgress = async (totalTime: number): Promise<void> => {
...
// This function simulates the results from your Python code
export const scanWebsite = async (url: string): Promise<ScanResult> => {
  try {
    ...
    if (scanResultsCache[normalizedUrl]) {
      ...
      await simulateScanningProgress(scanTime);
